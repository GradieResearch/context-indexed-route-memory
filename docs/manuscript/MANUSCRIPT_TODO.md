...
Complete a non-Exp13.2 manuscript-readiness hardening pass: keep Exp11-Exp13.1 as the internal spine, document reproducibility/statistical/figure gaps, and defer Exp13.2 to a separate analysis/import/alignment pass.

Claim: The repository is more navigable and internally aligned, but not submission-ready.
Evidence: Exp11, Exp12, Exp13, and Exp13.1 have local artifacts and canonical evidence docs; current audits identify missing confidence intervals, final figure scripts, command verification, prior-art import, and baseline integration.
Caveat: Exp13.2 is intentionally excluded from this pass and must not be used here to mark C12 complete.
Source path: `docs/manuscript/CLAIMS_AND_EVIDENCE.md`; `docs/repo_audit/REPRODUCIBILITY_AUDIT.md`; `docs/source_data/STATISTICAL_REPORTING_READINESS.csv`

## P0 - Required Before Manuscript Submission

| TODO | Reason | Related experiment | Source path | Target output |
|---|---|---|---|---|
| Complete separate Exp13.2 analysis/import/alignment pass. | Baseline integration is required, but intentionally out of scope here. | Exp13.2 and manuscript-level | `docs/manuscript/BASELINE_REQUIREMENTS.md`; `docs/manuscript/CLAIMS_AND_EVIDENCE.md` | Reviewed baseline claims, caveats, figures/tables, and source-data manifests. |
| Add seed-level confidence intervals and effect sizes. | Many claims cite aggregate means without manuscript-grade uncertainty. | Exp11-Exp13.1 | `docs/source_data/STATISTICAL_REPORTING_READINESS.csv`; `scripts/compute_seed_metric_summary.py` | Human-reviewed CI/effect-size tables tied to explicit claim groupings. |
| Create final reproducible paper figures. | Current figure plan cites generated analysis plots, not final panel scripts. | Exp11-Exp13.1 | `docs/manuscript/FIGURE_PLAN.md`; `docs/source_data/SOURCE_DATA_MANIFEST.csv` | Figure scripts plus source-data manifests for every retained panel. |
| Decide Exp14 manuscript placement and add final artifacts if retained. | Exp14 is now validated local evidence for symbolic latent context inference, but generated plots are not final figures and source-data mirrors were not created during import. | Exp14 | `docs/threads/experiment14_analysis_digest.md`; `experiments/experiment14_latent_context_inference/analysis/exp14_full_20260507_210712/exp14_summary.csv`; `docs/manuscript/FIGURE_PLAN.md` | Main/supplement decision, final figure script, and source-data mirror for retained panels. |
| Verify manuscript-critical run commands on a fresh checkout. | Commands were inspected, not rerun in this cleanup pass. | Exp11-Exp13.1 | `docs/repo_audit/REPRODUCIBILITY_AUDIT.md` | Command log with pass/fail, runtime, hardware, and expected outputs. |
| Import novelty/prior-art assessment as a local artifact. | C12 depends on prior-art positioning and a missing thread-referenced artifact. | Manuscript-level | local verification pending for `Pasted text.txt`; `docs/manuscript/BASELINE_REQUIREMENTS.md` | Future `docs/manuscript/NOVELTY_ASSESSMENT_IMPORTED.md` or equivalent cited artifact. |
...
| Non-Exp13.2 artifact/source-data/statistical readiness indexes. | Current audit artifacts exclude Exp13.2 and identify remaining gaps. | `docs/repo_audit/ARTIFACT_INDEX.csv`; `docs/source_data/SOURCE_DATA_MANIFEST.csv`; `docs/source_data/STATISTICAL_REPORTING_READINESS.csv` |
| Exp14 latent-context digest import. | Exp14 full, validation, and smoke runs are documented; C13 is added as promising symbolic latent-context evidence. | `docs/experiments/exp14_summary.md`; `docs/repo_audit/EXP14_ANALYSIS_IMPORT_REPORT.md`; `docs/threads/experiment14_analysis_digest.md` |
